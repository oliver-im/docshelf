(() => {
  ...
  window.addEventListener('message', event => {
    ...
    if (!data || typeof data !== 'object' || !('type' in data) || data.type !== 'docshelf-theme' || !('theme' in data)) return;
  ...
  document.addEventListener('click', event => {
    ...
    if (!(link instanceof HTMLAnchorElement) || !['', '_self', '_top'].includes(link.target.toLowerCase())) return;
