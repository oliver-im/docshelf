import { prepareAddition, addToShelf, prepareRemoval, removeFromShelf, prepareProjectRemoval, removeProjectFromShelf } from '../packages/local/shelf.mjs';
...
export function createRegistrationHandler({ root = docShelfRoot, getShelfPath = resolveShelfPath, getRoots = resolveSourceRoots, fallback = defaultShelfPath, local = localShelfPath } = {}) {
  return async body => {
    ...
    const keys = removing ? ['action', 'route', 'project', 'revision'] : ['action', 'sources', 'project', 'title', 'revision'];
    if (!body || !['preview', 'add', 'remove-preview', 'remove'].includes(body.action) || Object.keys(body).some(key => !keys.includes(key)) || removing && (body.route === undefined) === (body.project === undefined)) throw new Error('Send a registration request with selected paths, or a registered route or project to remove.');
