...
export const workspaceRoot = resolveWorkspaceRoot(process.env.DOCSHELF_WORKSPACE);
...
// Retain the loader's validated roots, including a workspace override, without
...
export async function loadShelf(options = {}) {
  ...
  return loadShelfFrom(shelfPath, options);
}
 ...
 * @param {{ workspaceRoot?: string }} [options] Overrides the configured workspace root.
 * @returns {Promise<Shelf>}
 */
export async function loadShelfFrom(shelfPath, options = {}) {
...
export async function artifactSourcesMatch(shelf, revisionState) {
  ...
  const roots = await sourceRootsForShelf(shelf).catch(() => null);
  if (!roots) return false;
  for (const artifact of shelf.artifacts) {
    if (artifact.format === 'claude') continue;
    if (!artifact.sourcePath) return false;
    const contents = await readArtifactSource(artifact, roots).catch(() => null);
...
async function readArtifactSources(shelf) {
  ...
  const roots = await sourceRootsForShelf(shelf);

  for (const artifact of shelf.artifacts) {
    if (artifact.format === 'claude') continue;
    if (!artifact.sourcePath) throw new Error(`Missing source path for ${artifact.route}.`);
    const contents = await readArtifactSource(artifact, roots);
...
/** @param {Shelf} shelf */
async function sourceRootsForShelf(shelf) {
  return shelfSourceRoots.get(shelf) || resolveSourceRoots();
}

/** Read the registered path again, not just its formerly canonical target.
 * A symlink can change while a sync waits for its lock or while a build runs.
 * @param {Artifact} artifact
 * @param {{ workspace: string, checkout: string }} roots
 */
async function readArtifactSource(artifact, roots) {
  const registeredPath = path.resolve(docShelfRoot, artifact.source);
  const checkedPath = async () => {
    const current = await realpath(registeredPath);
  ...
  const canonical = await checkedPath();
  ...
  try {
    ...
    const checkIdentity = async () => {
      ...
      if (!current.isFile() || opened.dev !== current.dev || opened.ino !== current.ino) {
        throw new Error(`Source changed while reading: ${artifact.source}`);
      }
    };
    await checkIdentity();
