...
| Custom webview creation | The `0.2.1` hosted scan rejected the local lint exception. Version `0.2.2` extends the element tag map with Electron's webview type, uses Obsidian's `createEl`, and adopts the detached element into the owning view's document. The guest is attached only after its private session, sandbox, and navigation guard are configured. |
