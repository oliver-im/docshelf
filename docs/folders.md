...
Both apps show a project’s folder documents as a tree that mirrors the disk. Each local document sits under the outermost watched folder of its own project that contains it, including individually added files inside that folder. Remote documents and files outside the project’s folders stay at the project root. When all of a project’s documents come from one folder, that folder’s own name is omitted; otherwise it appears by name, or by its registered source when two of the project’s folders share a name. Folders come first, alphabetically, and a folder whose only content is one subfolder shares its row, as in `guides/v2`. The tree only changes how the shelf is displayed: registrations, routes, and files stay the same, and documents cannot be moved between folders.

In Obsidian, folder rows collapse like project headings and remember their state with the workspace. Dragging or **Move up**/**Move down** reorders a document only among the documents in its own folder, and a folder row only among the folders beside it; folders stay above documents. Search results show each document’s folder path after its project. On the web, choosing a document opens any collapsed folders around it.
