...
To remove a whole project, right-click its heading and choose **Remove from shelf…**. This removes the project’s watched folders and every document listed under it, including individually added files and remote documents; the confirmation shows how many of each. Originals stay untouched. Other projects’ watched folders exclude the removed folders and files by exact path, so they do not reappear there even when files are added later, and folders registered under other projects stay on the shelf even when they sit inside a removed folder. Both apps support this action; on the web it requires the local watcher.

To remove one folder, right-click its row in the [folder tree](#folder-tree) and choose **Remove from shelf…**. This removes the documents listed under that row, including individually added files, and any of the project’s own watched folders inside it; the confirmation shows how many. Every registered folder that contains it, in any project, excludes it by exact path, so files added to it later stay off the shelf too. Originals stay untouched, and folders registered under other projects inside it stay on the shelf. To restore it, add the folder again or delete its `./` entry from `exclude`. Both apps support this action; on the web it requires the local watcher. Update the Obsidian plugin together with the web app, because older plugin versions do not recognize files inside an excluded folder as removed when saving an open draft.
...
In Obsidian, right-click a project heading and choose **Reset to alphabetical** to reset the order of its folders and documents. **Reset project order** resets the order of project headings. Both actions leave shelf registrations unchanged.
...
Both apps show a project’s folder documents as a tree that mirrors the disk. Each local document sits under the outermost watched folder of its own project that contains it, including individually added files inside that folder. Remote documents and files outside the project’s folders stay at the project root. Explicitly registered folders keep their own rows, named by their basename or by their registered source when names collide. An empty registered folder shows **No documents**, keeps its project heading visible, and remains watched until removed with **Remove from shelf…**. Empty subfolders without their own registration stay hidden. Folders come first, alphabetically, and an unregistered folder whose only content is one unregistered subfolder shares its row, as in `guides/v2`; rows never merge across a registration boundary. The tree only changes how the shelf is displayed: registrations, routes, and files stay the same, and documents cannot be moved between folders.

In Obsidian, folder rows collapse like project headings and remember their state with the workspace. Folder identities stay the same when documents appear or disappear or display labels change, preserving saved state and removal targets. Dragging or **Move up**/**Move down** reorders a document only among the documents in its own folder, and a folder row only among the folders beside it; folders stay above documents. Search results show each document’s folder path after its project. On the web, choosing a document opens any collapsed folders around it.
