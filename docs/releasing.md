...
DocShelf Web's first release is `v0.1.0`, and DocShelf for Obsidian's prepared release is `0.3.0`, with a minimum Obsidian version of `1.13.7`. The two apps share a repository but are versioned and released independently.
...
```sh
gh workflow run release-obsidian.yml --ref main -f version=0.3.0
```
