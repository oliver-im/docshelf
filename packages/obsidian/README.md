...
After **Remove from shelf…**, add the same file again to reconnect its open drafts. Drafts follow the source path even if re-adding changes the shelf route. Reopening the document can also recover a closed draft; recovery review and external-change checks still apply. See [adding and removing documents](../../docs/folders.md).
...
With no search query, documents are grouped by their registered project label and shown as compact rows with a file-type icon and title. Documents from watched folders nest under folder rows that mirror the disk. Empty registered folders remain visible with a **No documents** label and can be removed through the same menu; see [folder tree](../../docs/folders.md#folder-tree). Long titles are truncated; hover over a row to see its full title and source type. Descriptions stay hidden while browsing. Search results include short content excerpts. Click a project heading or folder row to collapse or expand it, or drag a project heading to reorder projects. Drag a document row to reorder it within its project folder, or a folder row among the folders beside it. Right-click a heading, folder, or document (or press **Shift+F10** while it is focused) for **Move up** and **Move down**. Project heading menus offer **Reset to alphabetical** for the folders and documents inside that project and **Reset project order** for the headings themselves. Project, folder, and document menus include **Remove from shelf…** with confirmation; removing a project removes its watched folders and all of its documents from the shelf, and removing a folder keeps files added to it later off the shelf too. Obsidian saves orders and collapse state with the workspace; new projects and documents follow their saved lists alphabetically. Document orders use stable registration routes, so renaming a title preserves its position. Reordering does not change registrations or source files, and documents cannot be dragged between projects or folders. Search shows matches across all projects, including collapsed ones, ordered by relevance with dragging disabled. Local file changes refresh automatically. For a manual refresh or setup, use **DocShelf: Reload shelf and documents** or **DocShelf: Configure shelf** in Obsidian's command palette.
